...
export const submitSolve = async (req, res) => {
  ...
  try {
    ...
    const xpEarned = correct ? (challenge.points || 50) : 0;
    ...
    // 5. Update student XP + weekly_xp if correct
    if (correct) {
      ...
      await supabase
        ...
        .update({
          ...
          weekly_xp: (student?.weekly_xp || 0) + xpEarned,  // ← also update weekly
        })
        .eq("user_id", userId);
    }
    ...
    return res.json({
      ...
      xpEarned,
      solution:     challenge.solution || null,   // ← uses `solution`
    });
